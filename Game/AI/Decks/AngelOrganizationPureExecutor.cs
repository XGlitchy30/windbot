...
{
    [Deck("AngelOrganizationPure", "AI_AngelOrganizationPure")]
    class AngelOrganizationPureExecutor : DefaultExecutor
    {
        public class CardId
        {
            public const int AOrgAmans = 16599452;
            public const int AOrgMysty = 16599453;
            public const int AOrgNesto = 16599459;
            public const int AOrgZagar = 16599458;
            public const int AOrgHyper = 16599457;
			public const int AOrgVerlo = 16599460;
            public const int AOrgSiguas = 16599456;

            public const int ArcarumJustice = 99189323;
            public const int MelodiousSolo = 14763299;
            public const int MelodiousAria = 40502912;
            public const int ArchlordKristya = 59509952;

            public const int Downbeat = 96540;
            public const int Reload = 22589918;
            public const int CalledbyTheGrave = 24224830;
            public const int CelestialTransform = 28890974;
            public const int ChaosZone = 94243005;

            public const int Imperma = 10045474;
            public const int AOrgUltimatum = 16599461;
    
            public const int AOrgPhoenatura = 16599463;
            public const int AOrgLeviaqua = 16599464;
            public const int AOrgLiber = 16599466;
            public const int AOrgSceptrum = 16599471;
            public const int AOrgMagrubia = 16599467;
            public const int AOrgTrisalba = 16599468;
            public const int TimelordVulgate = 67508932;
            public const int CondemnedDarklord = 35306215;
            public const int AOrgCras = 16599470;
            public const int Needlefiber = 50588353;
            public const int Apollousa = 4280258;
            public const int BorreloadDragon = 31833038;
        }

        public AngelOrganizationPureExecutor(GameAI ai, Duel duel)
            : base(ai, duel)
        {
			//AOrg Monsters effects
            AddExecutor(ExecutorType.Activate, CardId.AOrgAmans, AOrgAmansEffect);
            AddExecutor(ExecutorType.Activate, CardId.AOrgMysty, AOrgMystyEffect);
			AddExecutor(ExecutorType.Activate, CardId.AOrgVerlo, AOrgVerloEffect);
			AddExecutor(ExecutorType.Activate, CardId.AOrgNesto, AOrgNestoEffect);
			AddExecutor(ExecutorType.Activate, CardId.AOrgHyper, AOrgHyperEffect);
			AddExecutor(ExecutorType.Activate, CardId.AOrgZagar, AOrgZagarEffect);
			//default counters
            AddExecutor(ExecutorType.Activate, CardId.CalledbyTheGrave, DefaultCalledByTheGrave);
			AddExecutor(ExecutorType.Activate, CardId.Imperma, DefaultInfiniteImpermanence);
			//enable NS
			AddExecutor(ExecutorType.Summon, CardId.AOrgAmans, AOrgFirstNSummon);
			AddExecutor(ExecutorType.Summon, CardId.AOrgMysty, AOrgFirstNSummon);
			AddExecutor(ExecutorType.Summon, CardId.AOrgZagar, AOrgTributeSummon);
			AddExecutor(ExecutorType.Summon, CardId.AOrgSiguas, AOrgTributeSummon);
			AddExecutor(ExecutorType.Summon, CardId.ArchlordKristya, AOrgTributeSummon);
			AddExecutor(ExecutorType.Summon, CardId.MelodiousSolo, SoloNSummon);
			AddExecutor(ExecutorType.Summon, CardId.MelodiousAria, AriaNSummon);
			AddExecutor(ExecutorType.Summon, CardId.AOrgNesto, NestoSummon);
			AddExecutor(ExecutorType.Summon, CardId.AOrgVerlo, VerloSummon);
			//spsummon choices
            AddExecutor(ExecutorType.SpSummon, CardId.AOrgMysty, AOrgMystySummon);
            AddExecutor(ExecutorType.SpSummon, CardId.AOrgSiguas, AOrgSiguasSummon);
			AddExecutor(ExecutorType.SpSummon, CardId.ArcarumJustice, JusticeSummon);
			AddExecutor(ExecutorType.SpSummon, CardId.MelodiousSolo, SoloSummon);
			AddExecutor(ExecutorType.SpSummon, CardId.ArchlordKristya, KristyaSummon);
			AddExecutor(ExecutorType.SpSummon, CardId.AOrgVerlo, VerloSummon);
			//spsummon ED choices
			AddExecutor(ExecutorType.SpSummon, CardId.AOrgLeviaqua, AOrgLeviaquaSummon);
			AddExecutor(ExecutorType.SpSummon, CardId.AOrgLiber, AOrgLiberSummon);
			AddExecutor(ExecutorType.SpSummon, CardId.AOrgCras, AOrgCrasSummon);
			AddExecutor(ExecutorType.SpSummon, CardId.AOrgSceptrum, AOrgSceptrumSummon);
			AddExecutor(ExecutorType.SpSummon, CardId.AOrgMagrubia, AOrgMagrubiaSummon);
			AddExecutor(ExecutorType.SpSummon, CardId.AOrgTrisalba, AOrgTrisalbaSummon);
			
			AddExecutor(ExecutorType.SpSummon, CardId.BorreloadDragon, BorreloadDragonSummon);
			AddExecutor(ExecutorType.SpSummon, CardId.CondemnedDarklord, CondemnedDarklordSummon);
			AddExecutor(ExecutorType.SpSummon, CardId.Needlefiber, NeedlefiberSummonFirst);
			AddExecutor(ExecutorType.SpSummon, CardId.Apollousa, Apollousa);
			//ED effects
            AddExecutor(ExecutorType.Activate, CardId.AOrgPhoenatura, AOrgPhoenaturaEffect);
            AddExecutor(ExecutorType.Activate, CardId.AOrgLeviaqua, AOrgLeviaquaEffect);
			AddExecutor(ExecutorType.Activate, CardId.AOrgLiber, AOrgLiberEffect);
			AddExecutor(ExecutorType.Activate, CardId.AOrgSceptrum, AOrgSceptrumEffect);
			AddExecutor(ExecutorType.Activate, CardId.AOrgMagrubia, AOrgMagrubiaEffect);
			AddExecutor(ExecutorType.Activate, CardId.AOrgTrisalba, AOrgTrisalbaEffect);
			
			AddExecutor(ExecutorType.Activate, CardId.BorreloadDragon, BorreloadDragonEffect);
            AddExecutor(ExecutorType.Activate, CardId.AOrgCras, AOrgCrasEffect);
			...
			AddExecutor(ExecutorType.Activate, CardId.Needlefiber, NeedlefiberEffect);
			//SpellsTraps
			AddExecutor(ExecutorType.Activate, CardId.AOrgUltimatum, AOrgUltimatumEffect);
			AddExecutor(ExecutorType.Activate, CardId.Downbeat, DownbeatEffect);
			AddExecutor(ExecutorType.Activate, CardId.Reload, ReloadEffect);
			AddExecutor(ExecutorType.Activate, CardId.CelestialTransform, CelestialTransformEffect);
			AddExecutor(ExecutorType.Activate, CardId.ChaosZone);
			
            AddExecutor(ExecutorType.SpellSet, CardId.AOrgUltimatum, AOrgUltimatumSet);
			AddExecutor(ExecutorType.SpellSet, CardId.Imperma, ImpermaSet);

            AddExecutor(ExecutorType.Repos, MonsterRepos);
        }

        private bool NormalSummoned = false;
        private bool ZagarUsed = false;
